...
| F4 | `warden/tools/clock-rehearsal.mjs` is stale. It calls `insertMint` without the payment nonce required since 2026-09-05, so it would crash, and it is Sepolia-only. | Open. Superseded for mainnet by `mainnet-fork-clock.mjs`. Update it or retire it. |
