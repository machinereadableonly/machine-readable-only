...
test("a seeded child still scans at both extremes of the echo ring", () => {
  ...
  // raster those marks are about the size of a module. Half that ink sits in
  // different places from the dot rule this replaced, so the pattern beside the
  // code genuinely changed and is not merely regrouped. tools/echo-decode-check.mjs is the
